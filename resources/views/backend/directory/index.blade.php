...
@section('title', __('Online business directory | Admin'))
...
<script type="text/javascript">
    $(function () {
        var table = $('#directory-table').DataTable({
            ...
            "drawCallback": function(settings) {
                $('.status-check').change(function() {
                    ...
                    $.ajax({
                        url: "{{url('/')}}/admin/directory/change-status/" + id + "/" + status,
                        method: "GET",
                        timeout: 0,
                        dataType: "json",
                        success: function() {
        ...
        $(document).on('click', '.delete', function(){
            directory_id = $(this).attr('id');
            $('#confirmModal').modal('show');
        });

        $('#ok_button').click(function(){
            $.ajax({
                url:"directory/delete-directory/" + directory_id,
