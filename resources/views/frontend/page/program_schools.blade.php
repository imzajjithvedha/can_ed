...
@section('title', 'Proxima Study | Program schools')
...
@section('content')
    <div class="container mt-5 program-schools">

        <h4 class="fw-bolder futura">Program schools</h4>

        <hr>

        <div class="mt-5">

            @if(count($filtered_schools) == 0)
                        @include('frontend.includes.not_found_title',[
                            'not_found_title' => 'There is no school conducting this program',
                            'not_found_description' => 'Please check with another program.'
                        ])
            @else
                    ...
                    <div class="row justify-content-between border mb-4 py-3 mx-0">
                        <div class="col-5 text-center">
                            @if($school->featured_image != null)
                                <a href="{{ route('frontend.single_school', [$school->id, $school->slug]) }}" class="text-decoration-none"><img src="{{ uploaded_asset($school->featured_image) }}" class="card-img-top img-fluid w-100" style="height: 15rem; object-fir: cover;" alt="..."></a>
                            @else
                                <a href="{{ route('frontend.single_school', [$school->id, $school->slug]) }}" class="text-decoration-none"><img src="{{ url('img/frontend/no_image.jpg') }}" alt="" class="img-fluid w-100" style="height: 15rem; object-fit: cover;"></a>
                            @endif
