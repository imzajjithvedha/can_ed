...
@section('content')
    ...
    <form action="{{ route('admin.schools.school_scholarship_faq_create') }}" method="POST" novalidate>
        {{csrf_field()}}
    ...
    <script>
        $(function () {
            var table = $('#faq-table').DataTable({
                processing: true,
                ajax: "{{route('admin.schools.get_school_scholarships_faq', $school->id)}}",
                serverSide: true,
                order: [[2, "asc"]],
                columns: [
        ...
        $(document).on('click', '.delete', function(){
            ...
            url = "{{ route('admin.schools.school_scholarship_faq_delete', [$school->id, ':faq_id']) }}";
